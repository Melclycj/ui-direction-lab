...
# Maturity-aware weighted bar — mirrors the floor logic so a 'mature' utility page is NOT
# held to a showcase-grade 9.0 and need not be over-designed to pass. Overridable via --quality-bar.
QUALITY_BAR_BY_MATURITY = {
    "mature":   8.5,
    "creative": 8.75,
    "marquee":  9.0,
}
...
def main() -> int:
    ...
    p.add_argument("--quality-bar", type=float, default=None,
                   help="Override the maturity-aware weighted bar (mature 8.5 / creative 8.75 / marquee 9.0)")
    ...
    maturity = (contract or {}).get("surface_innovation_target", "mature")
    quality_bar = (args.quality_bar if args.quality_bar is not None
                   else QUALITY_BAR_BY_MATURITY.get(maturity, 8.5))
    audit = score(audit, quality_bar, weights, contract)
